...
> 原版基于 Node.js + Rust N-API (~5,362 行)，本版使用 Python + usearch + numpy 以等价思路重写，且无需编译 Rust。

当前仓库已经补齐一套可直接联调的完整链路：

- TagMemo 主服务：OpenAI 风格聊天接口、VCP 兼容接口、纯记忆查询接口
- 内置页面：`/chat` 聊天页、`/admin` 管理后台
- 审计与观测：结构化日志、SQLite 可视化查询
- Provider Proxy：可把外部 CLI 或 SDK 的原生协议接到 TagMemo 记忆层上
- Gemini Proxy：先查 TagMemo 记忆，再把增强后的原生 Gemini 请求转发到官方 Gemini
...
- **VCP 兼容闭环**：补齐原项目依赖的协议外皮、工具循环配套接口与过程信息透出能力
- **聊天前端**：新增内置 `/chat` 页面，支持流式和非流式调用主服务
- **后台管理**：新增 `/admin` 页面，用于查看查询日志、数据库表、日记文件与记忆预览
- **审计日志**：查询过程会写入 JSONL 和 SQLite，便于排障与回溯
- **Gemini 外挂记忆代理**：新增 `proxy/gemini/server.py`，支持 Gemini 原生协议接入 TagMemo 记忆层
...
如果要把 Gemini CLI 接到 TagMemo 作为外挂记忆库，请额外参考 [proxy/README.md](proxy/README.md) 与 [proxy/gemini/README.md](proxy/gemini/README.md)。

## API 端点

TagMemo-py 对外暴露的 API 覆盖主聊天、VCP 兼容、纯记忆查询和后台观测：
...
| `/v1/human/tool` | POST | VCP/工具调用兼容辅助接口 |
...
# VCP 兼容聊天接口
curl -X POST http://localhost:3100/v1/chatvcp/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o",
    "messages": [{"role": "user", "content": "总结一下我之前提过的研究方向"}],
    "stream": true,
    "force_show_vcp": true
  }'
...
当外部客户端并不使用 OpenAI 风格协议，而是直接发送某个 Provider 的原生协议时，可以通过 `proxy/` 目录下的适配器把请求先接入 TagMemo 的记忆层，再转发给对应上游。

当前目录布局：

- `proxy/common/`：抓包、调试、请求观察工具
- `proxy/gemini/`：Gemini CLI / Gemini API 协议适配
- `proxy/antigravity/`：预留给 Antigravity
- `proxy/copilot/`：预留给 Copilot

Gemini 代理的当前工作流是：

1. 接收 Gemini 原生 `generateContent` / `streamGenerateContent` 请求
2. 从 `contents` 中提取最新用户问题与历史上下文
3. 调用 TagMemo `POST /v1/memory/query`
4. 把返回的 `memory_context` 注入 Gemini `systemInstruction`
5. 将增强后的请求转发到官方 Gemini

这条链路的目标是：**TagMemo 只负责记忆检索，最终回答仍由官方 Gemini 生成。**
...
- 主服务 `config.env` 中的聊天上游配置仍用于 OpenAI 风格聊天链路
- Gemini 代理是独立进程，不会替代主服务的 `/v1/chat/completions`
- 若把 Gemini CLI 指向本项目，请优先让代理自身持有官方 Gemini key，避免依赖客户端转发认证头
